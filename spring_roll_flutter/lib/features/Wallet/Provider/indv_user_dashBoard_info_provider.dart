...
import 'package:spring_roll_flutter/features/Wallet/Model/fetch_indv_user_model.dart';
...
class IndvUserDashBoardInfoProviderNotifier
  ...
  Future<void> indvInfoProvider() async {
    ...
    final storedNumber = ref.read(phoneNumberProvider);
    final storedToken = ref.read(tokenStorageProvider);
    print("the url is hehehehakjdbhsfa ___$url:::$storedNumber");
    print("the number is ___$storedNumber");
    final queryParams = {
      'number': storedNumber,
    };
    try {
      final String cleanedToken =
          storedToken?.replaceAll(RegExp(r'token:|[{}]'), '').trim() ?? '';
